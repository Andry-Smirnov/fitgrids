...
type
  ...
  TGridEditingFinished = procedure(Sender: TObject; Col, Row: LongInt) of object;

  TGridModified = procedure(Sender: TObject) of object;

  TClipboardGrid = class(TStringGrid)
  protected
    function CheckingTextValidity(St: string; ACol, ARow: LongInt): Boolean; virtual;
  ...
  protected
    ...
    // Coordinates of edited cell.
    procedure EditingFinished(const ACol, ARow: LongInt); virtual;
    // Call OnGridModified.
  ...
  published
    ...
    property OnGridModified: TGridModified read FGridModified write FGridModified;
  end;
  ...
  public
    ...
    // Clear cells of selected region.
  ...
  published
    ...
    property SelectedRegionColor: TColor read GetSelectedRegionColor write SetSelectedRegionColor;
  ...
  public
    ...
    property CellsColors[ACol, ARow: LongInt]: TColor read GetCellsColors write SetCellsColors;
  published
    property DoubleBuffered;
    ...
    property SelectedRegionColor: TColor read FSelectedRegionColor write SetSelectedRegionColor;
    property ColNumFixed: Boolean read FColNumFixed write FColNumFixed;
    ...
    // from clipboard.
    property RowNumFixed: Boolean read FRowNumFixed write FRowNumFixed;
    property OnGetCellColor: TGetCellColorEvent read FOnGetCellColor write FOnGetCellColor;
  end;
  ...
  protected
    ...
    procedure SetColCount(Value: Longint); override;
    function CheckingTextValidity(St: string; ACol, ARow: LongInt): Boolean; override;
  ...
  public
    ...
    property ColOptions[index: LongInt]: TColOption read GetColOption write SetColOption;
  published
...
const
  ...
  SelectOptions: set of TGridOption = [goFixedVertLine, goFixedHorzLine,
    goVertLine, goHorzLine, goRangeSelect, goDrawFocusSelected, goTabs,
    goThumbTracking, goRowSizing, goColSizing];

  EditingOptions: set of TGridOption = [goFixedVertLine, goFixedHorzLine,
    goVertLine, goHorzLine, goEditing, goDrawFocusSelected, goTabs,
    goThumbTracking, goAlwaysShowEditor, goRowSizing, goColSizing];

  StaticOptions: set of TGridOption = [goFixedVertLine, goFixedHorzLine,
    goVertLine, goHorzLine, goTabs, goThumbTracking, goRowSizing, goColSizing];
...
begin
  ...
  RegisterPropertyEditor(TypeInfo(TColor), TColorStringGrid, 'OddRowColor', TColorProperty);
  RegisterPropertyEditor(TypeInfo(TColor), TColorStringGrid, 'EvenRowColor', TColorProperty);
  RegisterPropertyEditor(TypeInfo(TColor), TColorStringGrid, 'SelectedRegionColor', TColorProperty);
  RegisterPropertyEditor(TypeInfo(TColor), TColorStringGrid, 'DisabledColor', TColorProperty);
  RegisterPropertyEditor(TypeInfo(Boolean), TColorStringGrid, 'RowNumFixed', TEnumProperty);
  RegisterPropertyEditor(TypeInfo(Boolean), TColorStringGrid, 'ColNumFixed', TEnumProperty);
  RegisterPropertyEditor(TypeInfo(Boolean), TColorStringGrid, 'DoubleBuffered', TEnumProperty);
  RegisterPropertyEditor(TypeInfo(TGetCellColorEvent), TColorStringGrid, 'OnGetCellColor', TMethodProperty);

  RegisterPropertyEditor(TypeInfo(TColor), TColoredGrid, 'OddRowColor', TColorProperty);
  RegisterPropertyEditor(TypeInfo(TColor), TColoredGrid, 'EvenRowColor', TColorProperty);
  RegisterPropertyEditor(TypeInfo(TColor), TColoredGrid, 'SelectedRegionColor', TColorProperty);
  RegisterPropertyEditor(TypeInfo(TColor), TColoredGrid, 'DisabledColor', TColorProperty);

  RegisterPropertyEditor(TypeInfo(Boolean), TIDAGrid, 'DoubleBuffered', TEnumProperty);
  RegisterPropertyEditor(TypeInfo(Boolean), TIDAGrid, 'RowNumFixed', TEnumProperty);
  RegisterPropertyEditor(TypeInfo(Boolean), TIDAGrid, 'ColNumFixed', TEnumProperty);
  RegisterPropertyEditor(TypeInfo(Boolean), TIDAGrid, 'Changeable', TEnumProperty);
  RegisterPropertyEditor(TypeInfo(TGridResizedEvent), TIDAGrid, 'OnGridResized', TMethodProperty);

  RegisterPropertyEditor(TypeInfo(TGridEditingFinished), TGEFGrid, 'OnGridEditingFinished', TMethodProperty);
  RegisterPropertyEditor(TypeInfo(TGridModified), TGEFGrid, 'OnGridModified', TMethodProperty);
end;
  ...
  if SelFlag then
    begin
      ...
      if (Coord.X <> SavedCoord.X) or (Coord.Y <> SavedCoord.Y) then
        begin
          if (StartCoord.Y <= FixedRows - 1)
            and (StartCoord.X >= FixedCols)
            and (Coord.X >= FixedCols) then
            begin
              ...
              Selection := R;
              if (Coord.X - LeftCol = VisibleColCount)
                and (Coord.X < ColCount - 1) then
                LeftCol := LeftCol + 1;
              if (Coord.X = LeftCol)
                and (LeftCol > FixedCols) then
                LeftCol := LeftCol - 1;
            end;

          if (StartCoord.X <= FixedCols - 1)
            and (StartCoord.Y >= FixedRows)
            and (Coord.Y >= FixedRows) then
            begin
              //  Rows are selected.
              R.Left := FixedCols;
              R.Right := ColCount - 1;
              ...
              Selection := R;
              if (Coord.Y - TopRow = VisibleRowCount)
                and (Coord.Y < RowCount - 1) then
                TopRow := TopRow + 1;
              if (Coord.Y = TopRow)
                and (TopRow > FixedRows) then
                TopRow := TopRow - 1;
            end;
          SavedCoord := Coord;
...
begin
  ...
  if (Selection.Left = FixedCols) and (Selection.Right = ColCount - 1) and
    ...
    begin
      if MayIDoDeleteAllData then
        begin
          if MessageDlg('Clear table ?', mtConfirmation, [mbYes, mbNo, mbCancel], 0) = mrYes then
            begin
              DeleteAllData;
  ...
  if (Selection.Left = FixedCols) and (Selection.Right = ColCount - 1) then
    begin
      ...
      else if MayIDoClearSelectedArea then
        begin
...
procedure TColoredGrid.DrawCell(ACol, ARow: Longint; ARect: TRect; AState: TGridDrawState);
    ...
    begin
      ...
      if not (gdFixed in AState) then
        ...
        begin
          ...
          else if GetMyGridDataSource <> nil then
            begin
...
begin
  ...
  if (FGridDataSource <> nil) and
     (GridDataSource <> FGridDataSource) then
     SaveTableParams;
...
var
  ...
  St: string;
  Index: LongInt;
  BufferColCount: LongInt;
  BufferRowCount: LongInt;
  TempCol: LongInt;
  TempRow: LongInt;
  i: LongInt;
  j: LongInt;
  SavedSelection: TGridRect;
  InsertedArea: TGridRect;
...
begin
  ...
  if MessageDlg('Overwrite this data ?', mtWarning, [mbYes, mbNo, mbCancel], 0) <> mrYes then
    Exit;
...
begin
  if Key >= #20 then
    case ColOptions[Col] of
      coReal: if Key in REAL_SET then
                Result := True
              else
                Result := False;
      coInteger: if Key in INT_SET then
                   Result := True
                 else
                   Result := False;
      coChars: if Key in CHAR_SET then
                 Result := True
               else
                 Result := False;
      coText: Result := True;
...
begin
  ...
  else if Assigned(FColorMatrix) then
    Result := CellsColors[ColNum, RowNum]
  else if Odd(RowNum) then
    Result := OddRowColor
  else
    Result := EvenRowColor;
end;
...
begin
  ...
  case Key of
    ...
    ',' : if ColOptions[Col] = coReal then //  Substitute decimal separator.
            Key := '.';
  end;
end;
      ...
      for i := 0 to RowCount - 1 do
        if Length(FColorMatrix[i]) <> Value then
          begin
            ...
            for j := SavedLength to
              Length(FColorMatrix[i]) - 1 do
            if Odd(i) then
...
begin
  if GetMyGridDataSource <> nil then
    Result := inherited MayIDoClearAllCells and GetMyGridDataSource.MayIDoClearAllCells
  else
...
begin
  if GetMyGridDataSource <> nil then
    Result := inherited MayIDoClearSelectedArea
      and GetMyGridDataSource.MayIDoClearSelectedArea
  else
    Result := inherited MayIDoClearSelectedArea;
end;
...
begin
  if GetMyGridDataSource <> nil then
    Result := inherited MayIDoDeleteAllData
      and GetMyGridDataSource.MayIDoDeleteAllData
  else
    Result := inherited MayIDoDeleteAllData;
end;
...
begin
  if GetMyGridDataSource <> nil then
    Result := inherited MayIDoDeleteColumns(StartCol, ColsCount)
      and GetMyGridDataSource.MayIDoDeleteColumns(StartCol, ColsCount)
  else
    Result := inherited MayIDoDeleteColumns(StartCol, ColsCount);
end;
...
begin
  if GetMyGridDataSource <> nil then
    Result := inherited MayIDoDeleteRows(StartRow, RowsCount)
      and GetMyGridDataSource.MayIDoDeleteRows(StartRow, RowsCount)
  else
    Result := inherited MayIDoDeleteRows(StartRow, RowsCount);
end;
...
begin
  if GetMyGridDataSource <> nil then
    Result := inherited MayIDoInsertColumns(StartCol, ColsCount)
      and GetMyGridDataSource.MayIDoInsertColumns(StartCol, ColsCount)
  else
    Result := inherited MayIDoInsertColumns(StartCol, ColsCount);
end;
...
begin
  if GetMyGridDataSource <> nil then
    Result := inherited MayIDoInsertRows(StartRow, RowsCount)
      and GetMyGridDataSource.MayIDoInsertRows(StartRow, RowsCount)
  else
    Result := inherited MayIDoInsertRows(StartRow, RowsCount);
end;
